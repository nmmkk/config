...
before_configure () {
  ...
  if [ ! -f /usr/include/sys/stropts.h -a \
       -f /usr/include/stropts.h ]; then
    mylog echo ""
    mylog echo "# Replacing 'sys/stropts.h' with 'stropts.h'"
    for f in process.c pty.c screen.c tty.sh
    do
        mylog echo "-- Modifiyng ${f}"
        sed -i 's/<sys\/stropts.h>/<stropts.h>/g' "${f}"
    done
  fi
